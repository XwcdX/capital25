...
<div class="h-screen w-screen relative flex items-center justify-center">
    ...
    <div class="absolute top-0 right-0 my-3 mr-3 flex space-x-3 flex-wrap ">
        {{-- greenpoint --}}
        <button onclick="openModal('greenpointModal'); hideText('homepage_text')" 
            class="hover:bg-slate-400 hover:cursor-pointer bg-white text-[#3e5c49] rounded-[2vw] w-[10rem] min-w-[6rem] h-12 flex items-center justify-center box-border px-3">
            <img src="{{ asset('assets/lifecycleHPDummy/leafDummy.png') }}" class="w-5 h-5 mr-3">
            <span class="text-2xl">{{$greenpoint}}</span>
        </button>

        {{-- coins --}}
        <button onclick="openModal('coinModal'); hideText('homepage_text')" 
            class="hover:bg-slate-400 hover:cursor-pointer bg-white text-[#3e5c49] rounded-[2vw] w-[10em] min-w-[6rem] h-12 flex items-center justify-center box-border px-3">
            <img src="{{ asset('assets/lifecycleHPDummy/coinDummy.png') }}" class="w-5 h-5 mr-3">
            <span class="text-2xl">{{$coin}}</span>  
        </button>
    </div>

    {{-- Lifecycle simulation text --}}
    <div class="z-10 text-center" id="homepage_text">
        <h1 class="text-3xl sm:text-5xl text-white font-semibold">Lifecycle Simulation</h1>
        <h1 class="text-4xl sm:text-6xl text-white font-bold">Capital 2025</h1>
    </div>
</div>

{{-- Modal untuk Greenpoint --}}
<div id="outer">
    <div id="greenpointModal" class="hidden fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center">
        ...
        <div class="modal-content bg-white rounded-[2.5vw] w-[90%] sm:w-[55rem] h-auto max-h-[90vh] flex flex-col items-center px-[8rem] pt-[2.3rem] pb-[2rem]">
            ...
            <div class="flex flex-row mb-3.5">
                <img src="{{ asset('assets/lifecycleHPDummy/leafDummy.png') }}" class="w-8 h-8 mr-2">
                <span class=" text-3xl text-[#3e5c49]">{{$greenpoint}}</span>
            </div>
            {{-- Riwayat Transaksi --}}
            <div class="h-[50vh] sm:h-[18rem] w-full sm:w-[40rem] overflow-y-auto rounded-l-[2vw] rounded-r-[1.5vw] bg-[#3e5c49] custom-scrollbar">
    ...
    <div class="flex justify-between items-center text-white font-semibold text-2xl ml-5 mr-5 mb-5">
        {{-- Tanggal - Jam flex-col --}}
        <div class="flex flex-col">
            {{-- Tanggal - Value flex-row --}}
...
<div id="outer" >
    <div id="coinModal" class="hidden fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center">
        ...
        <div class="modal-content bg-white rounded-[2.5vw] w-[90%] sm:w-[55rem] h-auto max-h-[90vh] flex flex-col items-center pt-[2.3rem] pb-[2rem]">
            {{-- icon + jumlah --}}
            <div class="flex flex-row mb-3.5">
                <img src="{{ asset('assets/lifecycleHPDummy/coinDummy.png') }}" class="w-8 h-8 mr-2">
                <span class=" text-3xl text-[#3e5c49]">{{$coin}}</span>
            </div>
            {{-- Riwayat Transaksi --}}
            <div class="h-[50vh] sm:h-[18rem] w-full sm:w-[40rem] overflow-y-auto rounded-l-[2vw] rounded-r-[1.5vw] bg-[#3e5c49] custom-scrollbar">
                <h1 class="text-white font-bold text-center mt-2 pl-5 text-lg sm:text-2xl">Riwayat Transaksi</h1>
